...
if st.session_state.sim_data is not None:
    ...
    with tab1:
        ...
        with mc1:
            ...
            st.divider()
            f0, f1, f2 = st.columns(3); f0.metric("CAPEX", f"{sd['capex_s1_tot']:.2f} €");
            f1.metric("Risparmio Economico" if lang=="ITA" else "Financial Savings", f"{sd['savings_s1']:.2f} €/anno"); f2.metric("Tempo di Ritorno (PBP)" if lang=="ITA" else "Simple Payback Period", f"{sd['payback_s1']:.1f} Anni" if lang=="ITA" else f"{sd['payback_s1']:.1f} Years")
    ...
    if has_ev:
        with tab2:
            ...
            with mc1:
                ...
                f1.metric("Risparmio Economico" if lang=="ITA" else "Financial Savings", f"{sd['savings_s2']:.2f} €/anno"); f2.metric("Tempo di Ritorno (PBP)" if lang=="ITA" else "Simple Payback Period", f"{sd['payback_s2']:.1f} Anni" if lang=="ITA" else f"{sd['payback_s2']:.1f} Years")
        ...
        with tab3:
            ...
            with mc1:
                ...
                f1.metric("Risparmio Economico" if lang=="ITA" else "Financial Savings", f"{sd['savings_s3']:.2f} €/anno"); f2.metric("Tempo di Ritorno (PBP)" if lang=="ITA" else "Simple Payback Period", f"{sd['payback_s3']:.1f} Anni" if lang=="ITA" else f"{sd['payback_s3']:.1f} Years")
    ...
    for tab_obj, season_name in season_mapping:
        ...
        with tab_obj:
            ...
            with c_fabb1:
                ...
                ax_meteo.set_ylabel('T [°C]', fontsize=5.8, color='blue')
                ax_meteo.tick_params(axis='y', labelsize=5.8, colors='blue')
                ax_meteo.plot(h_range, temp_s, label="Temp. Esterna" if lang=="ITA" else "Ext. Temp", color="blue", lw=1.6, linestyle=":")
    ...
    with c_an_f1:
        ...
        ax_ann_temp.set_ylabel('T [°C]', fontsize=5.8, color='blue')
        ax_ann_temp.tick_params(axis='y', labelsize=5.8, colors='blue')
        ax_ann_temp.plot(t_range, sim["temp"][s_idx:e_idx], color="blue", lw=1, linestyle=":", label="Temp. Esterna" if lang=="ITA" else "Ext. Temp")
